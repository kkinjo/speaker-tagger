...
export default async function run() {
  ...
  try {
    await newProject(page, "固有名詞のテスト");
    ...
    // まとめて貼り付け（GlossaryPanel 側。参加者側と同名ボタンがあるため最後の方を指定）
    await page.getByRole("button", { name: "まとめて貼り付け" }).last().click();
    await page
      .locator("textarea[placeholder^='1行に1語']")
      .fill("議案名A\n九附後連\n\n  前後の空白  ");
    await page.getByRole("button", { name: "この内容で追加" }).last().click();
    ...
    // 取り込み済みプロジェクトは再読み込み後ドロワーが閉じているので開き直す
